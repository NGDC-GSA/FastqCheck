...
#define __BLOOMFILTER_VERSION__ "1.2"
 ...
 *      (2) delet the function of "SaveBloomFilterToFile" and "LoadBloomFilterFromFile"
**/
...
typedef struct
{
    ...
    uint64_t dwMaxItems;                            // n - BloomFilter中最大元素个数 (输入量)
    double dProbFalse;                              // p - 假阳概率 (输入量，比如万分之一：0.00001)
    uint64_t dwFilterBits;                          // m = ceil((n * log(p)) / log(1.0 / (pow(2.0, log(2.0))))); - BloomFilter的比特数
    uint64_t dwHashFuncs;                           // k = round(log(2.0) * m / n); - 哈希函数个数
    
    uint64_t dwSeed;                                // MurmurHash的种子偏移量
    uint64_t dwCount;                               // Add()的计数，超过MAX_BLOOMFILTER_N则返回失败
...
typedef struct
{
    uint64_t dwMagicCode;                           // 文件头部标识，填充 __MGAIC_CODE__
    uint64_t dwSeed;
    uint64_t dwCount;
    ...
    uint64_t dwResv[6];
    uint64_t dwFileCrc;                             // (未使用)整个文件的校验和
...
// 计算BloomFilter的参数m,k
static  void _CalcBloomFilterParam(uint64_t n, double p, uint64_t *pm, uint64_t *pk)
{
    /**
...
// 根据目标精度和数据个数，初始化BloomFilter结构
 int InitBloomFilter(BaseBloomFilter *pstBloomfilter, uint64_t dwSeed, uint64_t dwMaxItems, double dProbFalse)
{
